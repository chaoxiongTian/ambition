...
object TransformUtils {
    fun bytes2Int(bytes: ByteArray?): Int {
        return if (null == bytes || bytes.isEmpty()) 0 else (bytes[0].toInt() and 0XFF shl 24
    ...
    fun bytes2Short(bytes: ByteArray?): Short {
        return if (null == bytes || bytes.isEmpty()) 0 else (bytes[0].toInt() and 0xff or (
    ...
    fun bytes2Long(bytes: ByteArray?): Long {
        ...
        val s0 = (bytes[0].toInt() and 0xff).toLong()
        var s1 = (bytes[1].toInt() and 0xff).toLong()
        var s2 = (bytes[2].toInt() and 0xff).toLong()
        var s3 = (bytes[3].toInt() and 0xff).toLong()
        var s4 = (bytes[4].toInt() and 0xff).toLong()
        var s5 = (bytes[5].toInt() and 0xff).toLong()
        var s6 = (bytes[6].toInt() and 0xff).toLong()
        var s7 = (bytes[7].toInt() and 0xff).toLong()
        s1 = s1 shl 8
        s2 = s2 shl 16
        s3 = s3 shl 24
        s4 = s4 shl 8 * 4
    ...
    fun long2Bytes(l: Long): ByteArray {
        ...
        val b = ByteArray(8)
        for (i in b.indices) {
            b[i] = java.lang.Long.valueOf(temp and 0xffL).toByte()
            temp = temp shr 8
        }
        return b
    }
